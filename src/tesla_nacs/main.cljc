(ns tesla-nacs.main
  "Kotodama WASM entrypoint for the Tesla_nacs clean-room actor (L4) — Clojure port.
  ...
  facts (`tesla_nacs.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "tesla_nacs")
(def tier "L4")
...
(def entity-specs
  [{:entity "Vehicle"      :plural "vehicles"      :id-prefix "teslanac_veh"
    :fields [:vin :model :state :odometerKm]       :required [:vin :model]
    :coerce {:odometerKm :float}       :refs {}}
   {:entity "Signal"       :plural "signals"       :id-prefix "teslanac_sig"
    :fields [:vehicleId :name :value :unit]        :required [:name :value]
    :coerce {:value :float}            :refs {:vehicleId "Vehicle"}}
   {:entity "ChargeSession" :plural "chargesessions" :id-prefix "teslanac_cha"
    :fields [:vehicleId :connectorId :energyKwh :status] :required [:energyKwh :status]
    :coerce {:energyKwh :float}        :refs {:vehicleId "Vehicle" :connectorId "Connector"}}
   {:entity "Connector"    :plural "connectors"    :id-prefix "teslanac_con"
    :fields [:stationId :standard :powerKw :status] :required [:standard :powerKw]
    :coerce {:powerKw :float}          :refs {}}
   {:entity "Trip"         :plural "trips"         :id-prefix "teslanac_tri"
    :fields [:vehicleId :distanceKm :startedAt]    :required [:distanceKm :startedAt]
    :coerce {:distanceKm :float}       :refs {:vehicleId "Vehicle"}}
   {:entity "Command"      :plural "commands"      :id-prefix "teslanac_com"
    :fields [:vehicleId :name :status]             :required [:name :status]
    :coerce {}                         :refs {:vehicleId "Vehicle"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"tesla_nacs.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "tesla_nacs-compat" :tier tier :entities entities} 200])
...
(defn start! [] :tesla_nacs-compat/ready)

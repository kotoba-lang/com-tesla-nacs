(ns tesla-nacs.main-test
  "Contract + behavioral test for the tesla_nacs-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "tesla_nacs-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'tesla-nacs.main-test)]
